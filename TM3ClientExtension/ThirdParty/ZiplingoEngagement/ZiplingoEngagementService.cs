    ...
    public class ZiplingoEngagementService : IZiplingoEngagementService
    {
        ...
        public async void CallOrderZiplingoEngagementTrigger(Order order, string eventKey, bool FailedAutoship)
        {
            try
            {
                ...
                if (eventSetting != null && eventSetting?.Status == true)
                {
                    ...
                    order.LineItems.ForEach(l => l.Image = settings.LogoUrl.Split("/CMS")[0] + l.Image);
            ...
            try
            {
                ...
                if (eventSetting != null && eventSetting?.Status == true)
                {
                    ...
                    OrderData data = new OrderData
                    {
                        ...
                        Discount = order.Order.Totals.Select(m => m.DiscountTotal).FirstOrDefault(),
                        PaymentMethod = CardLastFourDegit,
        ...
        public async void CallOrderZiplingoEngagementTriggerForAssociateRankAdvancement(AssociateRankAdvancement assoRankAdvancementInfo, string eventKey)
        {
            try
            {
                ...
                var eventSetting = _ZiplingoEngagementRepository.GetEventSettingDetail(eventKey);
                LogMessage += "Event setting fetched";
                if (eventSetting != null && eventSetting?.Status == true)
                {
                    var company = _companyService.GetCompany();
                    LogMessage += " -- Company Fetched";
                    var settings = _ZiplingoEngagementRepository.GetSettings();
                    LogMessage += " -- Settings Fetched";
                    int enrollerID = 0;
                    int sponsorID = 0;
                    ...
                    _customLogRepository.CustomErrorLog(assoRankAdvancementInfo.AssociateId, assoRankAdvancementInfo.Rank, LogMessage, "");
                    ...
                    _customLogRepository.CustomErrorLog(assoRankAdvancementInfo.AssociateId, assoRankAdvancementInfo.Rank, "Rank Advancement trigger Api called", "");
                }
            }
            catch (Exception e)
            {
        ...
        public async void CreateEnrollContact(Order order)
        {
            try
            {
                ...
                var associateSummary = _distributorService.GetAssociate(order.AssociateId).Result;
        ...
        public async void CreateContact(Application req, ApplicationResponse response)
        {
            try
            {
                ...
                var associateSummary = _distributorService.GetAssociate(req.AssociateId).Result;
                var ZiplingoEngagementRequest = new AssociateContactModel
                {
        ...
        public async void UpdateContact(Associate req)
        {
            try
            {
                ...
                //var associateAutoships = _orderService.GetOrdersByAssociateId(req.AssociateId, "").Result;
                var ZiplingoEngagementRequest = new AssociateContactModel
                {
                    AssociateId = AssociateInfo.Result.AssociateId,
                    AssociateType = AssociateInfo.Result.AssociateBaseType,
                    ...
                    ActiveAutoship = _orderService.GetOrdersByAssociateId(req.AssociateId, "").Result.Where(o => o.OrderType == OrderType.Autoship).Any()
                };
        ...
        public async void CreateAutoshipTrigger(Autoship autoshipInfo)
        {
            ...
            {
                _customLogRepository.CustomErrorLog(autoshipInfo.AssociateId, autoshipInfo.AutoshipId, $"{ClassName}.CreateAutoshipTrigger", $"Exception occured in attempting CreateAutoshipTrigger");
            }
        }
